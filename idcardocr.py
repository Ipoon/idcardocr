...
def get_nation(img):
    ...
    # img = Image.fromarray(cv2.UMat.get(red).astype('uint8'))
    # return get_result_fix_length(red, 2, 'chi_sim', '--psm 10')
    # return get_result_vary_length(red, 'chi_sim', img, '--psm 7')
    # nation = get_result_fix_length(red, 1, 'chi_sim', '--psm 10')
    # 优化 多个字符的情况
    try:
        nation = get_result_vary_length(red, 'chi_sim', img, '--psm 7')
...
def get_idnum_and_birth(img):
    ...
    birth = idnum_str[6:14]
    return idnum_str, birth
